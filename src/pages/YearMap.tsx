  ...
  return (
    <div className="page-frame">
      <header className="mb-8">
        <p className="page-kicker mb-2">Stats</p>
        <h1 className="serif-title page-title">统计</h1>
        <p className="mt-3 text-[15px] text-[#57534B]">用更清晰的方式回看阅读进度与札记输出。</p>
      </header>
      ...
      <section className="mb-6 grid gap-4 lg:grid-cols-4">
        {[
          ...
          <div key={card.label} className="warm-card flex items-center gap-4 rounded-[8px] px-5 py-4">
            <span className="grid h-10 w-10 place-items-center rounded-[8px] text-[20px] text-[#FFFDF8]" style={{ backgroundColor: card.color }}>{card.icon}</span>
            <div>
              <p className="text-[14px] text-[#57534B]">{card.label}</p>
              <p className="serif-title mt-1 text-[27px] font-semibold leading-none text-[#26241F]">
                {card.value} <span className="text-[14px] font-normal">{card.unit}</span>
              </p>
            </div>
          </div>
        ))}
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="warm-card rounded-[8px] p-6">
          <h2 className="serif-title mb-7 text-[23px] font-semibold text-[#26241F]">时间 · 阅读书目（月）</h2>
          <div className="space-y-6">
            {monthStats.map(([month, count]) => (
              <div key={month} className="grid grid-cols-[88px_1fr_42px] items-center gap-6 text-[15px]">
          ...
          <p className="mt-20 border-t border-dashed border-[#26241F]/14 pt-5 text-[13px] text-[#6F6A60]">
            ⓘ 按书目看札记条数和字数，回顾阅读与输出情况。
          </p>
        </div>

        <div className="warm-card rounded-[8px] p-6">
          <h2 className="serif-title mb-7 text-[23px] font-semibold text-[#26241F]">书目 · 笔记条数、字数</h2>
          <div className="space-y-6">
            {bookStats.map(({ book, noteCount, wordCount }) => (
              <div key={book.id}>
                <div className="mb-2.5 flex items-center justify-between gap-4">
                  <span className="serif-title truncate text-[16px] font-semibold text-[#26241F]">{book.title}</span>
                  <span className="flex-shrink-0 text-[14px] text-[#57534B]">{noteCount} 条 · {wordCount} 字</span>
                </div>
                <div className="h-1 rounded-full bg-[#E9E5DC]">
          ...
          <p className="mt-14 border-t border-dashed border-[#26241F]/14 pt-5 text-[13px] text-[#6F6A60]">
