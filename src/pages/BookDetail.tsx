...
export default function BookDetail() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <Link to="/shelf" className="text-[13px] text-[#7A7468] hover:text-[#3D3A32] transition-colors mb-6 inline-block">
        ← 书架
      </Link>
      ...
      {showEdit ? (
        ...
        <div className="flex gap-4 mb-8">
          <div className="w-3 rounded-sm flex-shrink-0" style={{ backgroundColor: book.cover_color, minHeight: '80px' }} />
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <h1 className="text-[22px] font-medium text-[#3D3A32] leading-snug mb-1"
            ...
            <p className="text-[14px] text-[#7A7468]">
              {book.author}
              ...
              {book.rating && (
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <span key={i} className={i < book.rating! ? 'text-[#C4A84B] text-[13px]' : 'text-[#7A7468]/25 text-[13px]'}>★</span>
                  ))}
                </div>
              )}
      ...
      {tab === 'quotes' && (
        <div>
          {showKindleImporter ? (
            ...
            <div className="bg-[#E8E3D8] border border-black/10 rounded-xl p-5 mb-4">
              <textarea placeholder="书摘原文 *" value={quoteContent} onChange={e => setQuoteContent(e.target.value)}
                rows={4}
                className="w-full bg-transparent resize-none outline-none text-[15px] leading-relaxed text-[#3D3A32] placeholder-[#7A7468] mb-3 border-b border-black/15 pb-3"
              ...
              <textarea placeholder="个人感想（选填）" value={quoteReflection} onChange={e => setQuoteReflection(e.target.value)}
                rows={2}
                className="w-full bg-transparent resize-none outline-none text-[14px] leading-relaxed text-[#3D3A32] placeholder-[#7A7468] mb-3" />
            ...
            <div className="flex gap-2 justify-end mb-4">
              <button onClick={() => setShowKindleImporter(true)}
                className="px-4 py-1.5 text-sm border border-[#3D3A32]/30 text-[#3D3A32] rounded-lg hover:border-[#3D3A32] transition-colors">
                导入 Kindle 书摘
              </button>
      ...
      {tab === 'sparks' && (
        <div>
          <div className="bg-[#E8E3D8] border border-black/10 rounded-xl p-4 mb-4">
          ...
          {sparks.length === 0 && <p className="text-[14px] text-[#7A7468] py-4">还没有灵感碎片。</p>}
