...
export default function ReviewEditor({ bookId, bookColor, userId, onSave, onCancel }: Props) {
  ...
  return (
    <div className="rounded-[8px] border border-black/10 bg-[#E8E3D8] p-3 md:p-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          {STAGES.map(item => (
            <button
              ...
              className={`rounded-[8px] border px-2 py-1.5 text-left transition-colors md:px-3 md:py-2 ${
                stage === item.key
            ...
            >
              ...
              <span className="mt-0.5 hidden text-[11px] leading-snug opacity-80 sm:block">{item.hint}</span>
            </button>
          ))}
        </div>
      </div>

      <div className="mb-3 grid gap-2 md:flex md:gap-3">
        <input
          value={label}
        ...
        <div className="flex items-center gap-1.5">
          {LAYER_COLORS.map(c => (
            <button key={c} onClick={() => setColor(c)}
              className="h-5 w-5 rounded-full border-2 transition-all md:h-4 md:w-4"
              style={{ backgroundColor: c, borderColor: color === c ? '#3D3A32' : 'rgba(0,0,0,0.1)' }} />
          ))}
        </div>
      </div>

      <div className="border-l-2 pl-3 md:pl-4" style={{ borderColor: color }}>
        <textarea
          ref={textareaRef}
          value={content}
          ...
          rows={4}
          className="w-full resize-none overflow-hidden bg-transparent text-[14px] leading-relaxed text-[#3D3A32] outline-none placeholder-[#7A7468] md:text-[15px]"
