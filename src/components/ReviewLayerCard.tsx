...
export default function ReviewLayerCard({ layer, onDelete }: Props) {
  ...
  return (
    <div className="rounded-[8px] border border-black/10 bg-[#E8E3D8] p-2.5 md:p-5">
      <div className="mb-2 flex items-start justify-between gap-3 md:mb-3">
        <div className="flex flex-wrap items-center gap-1.5 md:gap-2">
          ...
          <span className="text-[11px] text-[#7A7468] md:text-[12px]">{date}</span>
        </div>
      ...
      <div className="border-l-2 pl-2.5 text-[13px] leading-relaxed text-[#3D3A32] md:pl-4 md:text-[15px]" style={{ borderColor: layer.color, fontFamily: '"Georgia", "Noto Serif SC", serif' }}>
        {expanded
      ...
      <div className="mt-2 flex items-center justify-between md:mt-3">
        <span className="text-[11px] text-[#7A7468] md:text-[12px]">{layer.word_count} 字</span>
        {isLong && (
