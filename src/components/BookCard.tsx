...
export default function BookCard({ book, onDelete, onStatusChange }: Props) {
  ...
  return (
    <div className="overflow-hidden rounded-[8px] border border-[#312F2A]/10 bg-[#EDE4D3]">
      ...
      <div className="px-3 pb-2.5 pt-3 md:px-4 md:pb-3 md:pt-3.5">
        <div className="mb-1 flex items-start justify-between gap-2">
          <Link
            to={`/book/${book.id}`}
            className="serif-title flex-1 text-[15px] font-semibold leading-snug text-[#312F2A] transition-colors hover:text-[#496F8E] md:text-[16px]"
          >
            {book.title}
          </Link>
          <span
        ...
        <p className="mb-1.5 text-[12px] text-[#746E62] md:mb-2">
          {book.author}
        ...
        <div className="mt-2.5 flex items-center justify-between border-t border-[#312F2A]/8 pt-2 md:mt-3 md:pt-2.5">
