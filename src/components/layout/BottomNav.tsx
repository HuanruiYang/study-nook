...
const NAV_ITEMS = [
  ...
  { to: '/account', label: '账号', end: true },
]

export default function BottomNav() {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-20 md:hidden">
      <div className="surface grid grid-cols-5 rounded-t-[8px] border-x-0 border-b-0 px-1.5 pb-[calc(4px+env(safe-area-inset-bottom))] pt-1">
        {NAV_ITEMS.map(item => (
          <NavLink
            key={item.to}
            to={item.to}
